...
describe AvailabilityCalculator do
  ...
  let(:availability) { Availability.new(person_id: person.id, start_date: start_date, end_date: end_date, percent_allocated: 100) }
  ...
  context "with partial allocations" do
    context "with a single allocation covering the range" do
      let(:alloc_start) { start_date }
      let(:alloc_end) { end_date }
      let(:allocations) { [double(start_date: alloc_start, end_date: alloc_end, percent_allocated: 40)] }

      it "returns the remaining billable percentage" do
        result.should == [availability.similar(percent_allocated: 60)]
      end
    end
    ...
    context "with a single day allocation" do
      ...
      let(:allocations) do
        [
          double(start_date: start_date, end_date: middle_day - 1.day, percent_allocated: 100),
          double(start_date: middle_day, end_date: middle_day, percent_allocated: 40),
          double(start_date: middle_day + 1.day, end_date: end_date, percent_allocated: 100)
        ]
      end

      it "returns only one availability" do
        result.should == [availability.similar(start_date: middle_day, end_date: middle_day, percent_allocated: 60)]
      end
    end
    ...
    context "that falls entirely within the specified dates" do
      ...
      it "returns availabilites around the allocation" do
        ...
        result.second.should == availability.similar(start_date: alloc_start, end_date: alloc_end, percent_allocated: 60)
        result.third.should == availability.similar(start_date: alloc_end + 1.day, end_date: end_date)
      end
    end
    ...
    context "starting within and ending after the specified dates" do
      ...
      it "returns an availability ending on the allocation's start date" do
        result.should == [availability.similar(end_date: alloc_start - 1.day), availability.similar(start_date: alloc_start, percent_allocated: 60)]
      end
    end
  end
  ...
  context "with an existing allocation" do
    let(:alloc_start) { start_date }
    let(:alloc_end) { end_date }
    let(:allocations) { [double(start_date: alloc_start, end_date: alloc_end, percent_allocated: 100)] }
    ...
    context "with a single day allocation" do
      ...
      let(:allocations) do
        [
          double(start_date: start_date, end_date: middle_day - 1.day, percent_allocated: 100),
          double(start_date: middle_day, end_date: middle_day, percent_allocated: 100),
          double(start_date: middle_day + 1.day, end_date: end_date, percent_allocated: 100)
        ]
      end
    ...
    context "many allocations" do
      context "that don't overlap" do
        ...
        let(:allocations) do
          [
            double(start_date: base_date+1.day, end_date: base_date+1.day, percent_allocated: 100),
            double(start_date: base_date+3.days, end_date: base_date+3.days, percent_allocated: 100)
          ]
        end
      ...
      context "that overlap" do
        ...
        let(:allocations) do
          [
            double(start_date: base_date+1.day, end_date: base_date+3.days, percent_allocated: 100),
            double(start_date: base_date+2.days, end_date: base_date+3.days, percent_allocated: 100)
          ]
        end
